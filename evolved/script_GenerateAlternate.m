...
params2.distgen = @(x) genDistEuclidean(n, x(1), x(2), 300);
params2.lb = [2, 10];
params2.ub = [15, 100];
params2.intcon = [1,2];
...
for t = indices
    ...
    [xout2] = generatorFunction(targets(t,:), model, features, params2);

    quality = Inf*ones(instPerTarget,1);
    dists = cell(instPerTarget,1);
    projs = cell(instPerTarget,1);

    for i = 1:instPerTarget
        % generate instance using identified parameters
        dists{i} = genDistEuclidean(n, xout2(1), xout2(2), xout2(3));
        
        projs{i} = qap2proj(dists{i},params2.flowgen(),model,features);
